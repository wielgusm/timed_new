...
def find_q_basic(obs,obs_err,product='cphase',statistic='basic'):
    ...
    if product=='cphase':
        ...
        elif statistic=='basic':
            #obs_sigma = np.std(obs)
            obs_sigma = circular_std(obs)
            eps_thermal = np.mean(obs_err)
        elif statistic=='mc':
            obs_sigma = circular_std(obs)
            eps_thermal = eps_MC(obs_err)
        elif statistic=='wrong':
            obs_sigma = np.array(1.)
            eps_thermal = np.array(1.)
    elif product=='lcamp':
        if statistic=='basic':
            obs_sigma = np.std(obs)#place holder
            eps_thermal = np.mean(obs_err)#place holder
    n = len(obs)
    if n > 0:
